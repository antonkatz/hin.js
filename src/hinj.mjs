...
import {ACCESSOR, HINGES_ANCESTRY_PROP,   HINGES_FACTORY_PROP, RAW_STACK, IS_ASYNC, STARTING_VALUE} from "./consts.mjs";
...
export function hinj(starting = undefined) {
  ...
  builder.asCmd = (isSubCommand) => {
    const fn = isAsyncMode ? (_this, args, remappedPointer = pointer) => {
      // const startingValue =
  ...
  builder.async = (wfn) => {
    ...
    if (!stack) {
      ...
      const pfn = isAsyncMode ? currentStack : (_t, vP, p) => {
